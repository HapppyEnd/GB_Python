...
Реализуйте программу для управления
...
def check_multiplicity(amount: int) -> bool:
    ...
    if not isinstance(amount, int):
        logging.error('Введено некорректное значение. '
                      f'Должно быть int, введено {type(amount)}')
        raise ValueError(f'Введено некорректное значение {amount}.')
    if amount < 0:
        logging.error(f'Введено некорректное значение. {amount} '
                      'должно быть > 0.')
        raise ValueError(f'Введено некорректное значение {amount}.')
...
def withdraw(amount: int) -> None:
    ...
    if not isinstance(amount, int):
        ...
        raise ValueError(f'Введено некорректное значение: {amount}.')
    if amount < 0:
        logging.error(f'Некорректное значение. {amount} '
                      'должно быть > 0.')
        raise ValueError(f'Введено некорректное значение {amount}.')
